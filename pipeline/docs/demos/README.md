...
`pr-review` re-runs every done story's spec on every PR, so every demo here is
re-verified continuously, and screenshot changes on untouched stories are treated as
visual regressions.
